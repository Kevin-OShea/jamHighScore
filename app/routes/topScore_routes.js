...
router.post('/topScores', (req, res, next) => {
  // set owner of new topScore to be current user
  req.body.topScore.owner = req.user.id
...
router.patch('/topScores/:id', removeBlanks, (req, res, next) => {
  ...
  delete req.body.topScore.owner
...
router.delete('/topScores/:id', (req, res, next) => {
  TopScore.findById(req.params.id)
    .then(handle404)
    .then(topScore => {
      // throw an error if current user doesn't own `topScore`
      requireOwnership(req, topScore)
